...
#  endif
 ...
 // Get number of threads from command line
   ...
   omp_set_num_threads(num_threads);//添加对线程数的控制，通过命令行参数设置线程数。
   
   Get_args(argc-1, argv+1, &n, &n_steps, &delta_t, &output_freq, &g_i);//新增这些
...
#  endif
 ...
 // Parallelize the main time stepping loop使用omp循环，三个循环头部加入#...通过OpenMP将主时间
//步进循环并行化，每个线程处理一个或多个时间步。

#  pragma omp parallel for private(part) collapse(1)
...
#     ifdef COMPUTE_ENERGY
 ...
 // Use a critical section for energy computation to avoid race conditions也是加了临界区保护
#        pragma omp critical
	  {
        Compute_energy(curr, n, &kinetic_energy, &potential_energy);
...
#     ifndef NO_OUTPUT
      if (step % output_freq == 0)
    // Use a critical section for output to avoid concurrent writes 使用临界区保护
#        pragma omp critical
